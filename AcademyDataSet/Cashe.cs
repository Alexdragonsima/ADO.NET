...
namespace AcademyDataSet
{
	internal class Cashe
	{
		...
		public Cashe()
		{
			CONNECTION_STRING = ConfigurationManager.ConnectionStrings["DBMS_DDL"].ConnectionString;
		...
		public void Load()
		{
			...
			for (int i = 0; i < tables.Length; i++)
			{
				...
				Set.Tables[tables[i].Split(',')[0]].Columns;
		...
		public void Print(string table)
		{
			...
			if (hasParents(table))
			{
				...
				//DataColumn parent_column = Set.Tables[parent_table_name].Columns["direction_name"];
			...
			foreach (DataRow row in Set.Tables[table].Rows)
			{
				...
				//if (hasParents(table))
				//{
				//	DataRow parent_row = row.GetParentRow(Set.Tables[table].ParentRelations[0].RelationName);
		...
		bool hasParents(string table)
		{
			...
			////Console.WriteLine(Set.Relations.ToString());
			//for(int i =0;i<Set.Relations.Count;i++)
			//{
			//	if (Set.Relations[i].ChildTable.TableName == table) return true;
			//}
			//return false;
		}
		void Check()
